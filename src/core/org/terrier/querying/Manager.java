...
=======
		//diversifyResults(srq);
		algo1(srq);
>>>>>>> 0a6356df930a4545c4a5256f247c9d376ae00f5e
	 ...
	 * @param initialSrq
	 * @param subQueries
	 * @param subQueryRelevance Relevance of SubQuery[i] to initial query initialSrq
	 * @param lambda
	 * @param w
	 */
		...
		while(diversifiedDocumentCount < lambda){
			...
			for(int initialDocumentIndex=0; initialDocumentIndex < initialResultSet.getResultSize(); initialDocumentIndex++){
				...
				for(int subQueryIndex = 0; subQueryIndex < subQueries.size() ; subQueryIndex++){
					SearchRequest currentSubQuery = subQueries.get(subQueryIndex);
					updatedScores[initialDocumentIndex] += subQueryRelevance[subQueryIndex] * getScoreForDocumentId(initialResultSet.getDocids()[initialDocumentIndex], currentSubQuery) * queryMass[subQueryIndex];
					
				}
				updatedScores[initialDocumentIndex] = Math.pow(updatedScores[initialDocumentIndex], w);
				updatedScores[initialDocumentIndex] *=initialScores[initialDocumentIndex] ; 
			}
			
			int maxDocumentIndex =0;
			double maxDocumentScore =0;
			for(int initialDocumentIndex=0; initialDocumentIndex < initialScores.length; initialDocumentIndex++){
				if(updatedScores[initialDocumentIndex]>maxDocumentScore){
					maxDocumentScore = updatedScores[initialDocumentIndex];
